...
namespace Library
{
    public class DataBaseService
    {
        ...
        public List<Book> FindAdditionalLiterature(Book selectedBook)
        {
            List<Book> additionalLiterature = new List<Book>();

            // Проверяем, что жанр выбранной книги соответствует условиям
            //if (selectedBook.Genre.GenreId == 2 ||
            //    selectedBook.Genre.GenreId == 3 ||
            //    selectedBook.Genre.GenreId == 4 ||
            //    selectedBook.Genre.GenreId == 10)
            //{
                string query = @"
            SELECT 
                b.BookId, b.Title, b.Annotation, b.YearPublished, b.PageCount, b.ReadingRoomNumber, b.IsAvailable, b.ImageUrl,
            ...
            WHERE 
                ...
                using (NpgsqlConnection connection = new NpgsqlConnection(connectionString))
                {
                    ...
                    using (NpgsqlCommand command = new NpgsqlCommand(query, connection))
                    {
                        command.Parameters.AddWithValue("GenreId", selectedBook.Genre.GenreId);
            ...
            return additionalLiterature.Take(3).ToList();
        }
    }
}